...
class PredBat(hass.Hass, Octopus, Energidataservice, Stromligning, Fetch, Plan, Marginal, Execute, Output, UserInterface, GitHub):
    ...
    def _fetch_ashp_weather_forecast(self, latitude, longitude, forecast_days, http_timeout):
        """
    ...
    def _predict_ashp_hourly_kwh(self, row, base_temperature, intercept_kwh, hdd_slope_kwh_per_deg, wind_slope_kwh_per_ms, solar_slope_kwh_per_wm2, min_kwh, max_kwh):
        """
        Predict one hour of ASHP consumption from weather inputs.
        """
        heating_degree = max(base_temperature - row["temperature_2m"], 0.0)
        forecast_kwh = intercept_kwh
        forecast_kwh += heating_degree * hdd_slope_kwh_per_deg
        forecast_kwh += row["wind_speed_10m"] * wind_slope_kwh_per_ms
        forecast_kwh += row["shortwave_radiation"] * solar_slope_kwh_per_wm2
        forecast_kwh = max(min_kwh, forecast_kwh)
        forecast_kwh = min(max_kwh, forecast_kwh)
        return round(forecast_kwh, 4)

    def update_ashp_forecast_sensor(self, scheduled=True):
        """
        Publish a weather-derived ASHP load forecast sensor for Predbat load_forecast.
        """
        enabled = self.get_arg("ashp_forecast_enable", False, indirect=False)
        if not enabled:
            return

        now_utc = datetime.now(timezone.utc)
        update_every = max(int(self.get_arg("ashp_forecast_update_every", 60, indirect=False)), 15)
        if self.ashp_forecast_last_updated and (now_utc - self.ashp_forecast_last_updated).total_seconds() < (update_every * 60):
            return

        entity_id = self.get_arg("ashp_forecast_entity_id", "sensor.ashp_load_forecast", indirect=False)
        latitude = self.get_arg("ashp_forecast_latitude", None, indirect=False)
        longitude = self.get_arg("ashp_forecast_longitude", None, indirect=False)

        if latitude is None or longitude is None:
            if not self.ashp_forecast_warned:
                self.log("Warn: ASHP forecast enabled but ashp_forecast_latitude/ashp_forecast_longitude are missing")
                self.ashp_forecast_warned = True
            return
        ...
        try:
            ...
            if not self.ashp_forecast_warned:
                self.log("Warn: ASHP forecast latitude/longitude are invalid: {} {}".format(latitude, longitude))
                self.ashp_forecast_warned = True
            return

        forecast_days = max(int(self.get_arg("ashp_forecast_days", 2, indirect=False)), 1)
        http_timeout = max(int(self.get_arg("ashp_forecast_http_timeout", 30, indirect=False)), 5)

        base_temperature = float(self.get_arg("ashp_forecast_base_temperature", 12.0, indirect=False))
        intercept_kwh = float(self.get_arg("ashp_forecast_intercept_kwh", 0.0, indirect=False))
        hdd_slope_kwh_per_deg = float(self.get_arg("ashp_forecast_hdd_slope_kwh_per_deg", 0.0, indirect=False))
        wind_slope_kwh_per_ms = float(self.get_arg("ashp_forecast_wind_slope_kwh_per_ms", 0.0, indirect=False))
        solar_slope_kwh_per_wm2 = float(self.get_arg("ashp_forecast_solar_slope_kwh_per_wm2", 0.0, indirect=False))
        min_kwh = float(self.get_arg("ashp_forecast_min_kwh", 0.0, indirect=False))
        max_kwh = float(self.get_arg("ashp_forecast_max_kwh", 10.0, indirect=False))

        try:
            current_hour = now_utc.replace(minute=0, second=0, microsecond=0)
            hourly_rows = self._fetch_ashp_weather_forecast(latitude, longitude, forecast_days, http_timeout)
            results = {}
            external = []
            hourly_kwh = {}
            cumulative = 0.0
            ...
            for row in hourly_rows:
                ...
                if stamp <= current_hour:
                    continue
                forecast_kwh = self._predict_ashp_hourly_kwh(row, base_temperature, intercept_kwh, hdd_slope_kwh_per_deg, wind_slope_kwh_per_ms, solar_slope_kwh_per_wm2, min_kwh, max_kwh)
            ...
            attributes = {
                ...
                "model": {
                    ...
                    "intercept_kwh": intercept_kwh,
                    "hdd_slope_kwh_per_deg": hdd_slope_kwh_per_deg,
                    "wind_slope_kwh_per_ms": wind_slope_kwh_per_ms,
                    "solar_slope_kwh_per_wm2": solar_slope_kwh_per_wm2,
                    "min_kwh": min_kwh,
                    "max_kwh": max_kwh,
                },
                "source": {
            ...
            self.set_state_wrapper(entity_id, state=round(next_hour_kwh, 4), attributes=attributes)
            self.ashp_forecast_last_updated = now_utc
            self.ashp_forecast_warned = False
            self.log("ASHP forecast: Published {} hours to {} next hour {} kWh total {} kWh".format(len(external) - 1, entity_id, round(next_hour_kwh, 4), round(cumulative, 4)))
        except Exception as err:
            self.log("Warn: ASHP forecast update failed: {}".format(err))
    ...
    def reset(self):
        ...
        self.ashp_forecast_last_updated = None
        self.ashp_forecast_warned = False
        self.soc_kw = 0
        self.soc_percent = 0
        self.soc_max = 10.0
    ...
    def update_pred(self, scheduled=True):
        ...
        self.update_ashp_forecast_sensor(scheduled=scheduled)
